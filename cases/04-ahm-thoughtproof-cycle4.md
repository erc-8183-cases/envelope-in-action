...
- **Behavioural diagnostics (AHM).** Produced over an agent or wallet subject using cross-registry behavioural scoring (D-dimensional analysis, confidence enum, observation density). Verdict is grounded in observed on-chain behaviour across multiple sources, not in the reasoning of any single deliverable.
...
This case documents two on-chain ERC-8183 jobs run on the same AgentJobManager deployment (Base Sepolia, contract `0xC07CE789206CBEEC3A41D5CedBdA93B1024aaDdd`), with the AHM and ThoughtProof evaluator wallets in mirrored roles across the pair:
...
**Scoring.** AHM attests AHS 57 / Grade D with HIGH confidence based on 9 cumulative scans. The behavioural pattern is mid-range stability: the agent scored 57/D in 8 of 9 observations, with one transient transition to 64/C on 2026-04-15 driven by a D2 dimensional spike (51 → 62, computed from token-transfer activity). The D2 spike subsided in the following cycle and the agent returned to its baseline grade.
...
- The Technical generator treated AHM's standard 2D-mode weight renormalization (where the D3 weight is redistributed across D1 and D2 to sum to 1.0) as a computational error. Red-team verified the renormalization pathway as documented methodology, not arithmetic mistake.

- The Contrarian generator framed AHM as committing a "category error" by measuring agent quality through wallet-health signals. Red-team identified this as a misreading of this deliverable's stated scope — the Jeff CEO deliverable ran in 2D mode (D1 + D2), measuring wallet behavioural health specifically, not the agent's task-execution quality. The deliverable said so explicitly. AHM's broader product surface includes a D4 dimension (task-execution / outcome quality), but D4 requires task-completion signals not available for this subject and was therefore not invoked.

The arithmetic chain was independently verified: raw weighted sum (0.30 × 70 + 0.50 × 51 = 46.5), renormalized weighted sum (0.375 × 70 + 0.625 × 51 = 58.125), with EMA temporal smoothing (alpha = 0.6) across 9 observations producing convergence to AHS 57. The synthesis verdict landed at ALLOW (confidence 0.72, medium-high), with one surviving dissenting concern carried explicitly into the record: HIGH confidence at n=9 observations is aggressive against general epistemological standards.
